...
prompt_segment() {
  local mybg myfg
  [[ -n $1 ]] && mybg="$bg[$1]" || mybg="$reset_color"
  [[ -n $2 ]] && myfg="$fg[$2]" || myfg="$reset_color"
  if [[ $CURRENT_BG != 'NONE' && $1 != $CURRENT_BG ]]; then
    echo -n " %{$mybg%}%{$fg[$CURRENT_BG]%}$SEGMENT_SEPARATOR%{$myfg%} "
  else
    echo -n "%{$mybg%}%{$myfg%} "
  fi
  CURRENT_BG=$1
...
prompt_context() {
  ...
  if [[ "$user" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment black default "%(!.%{%fg[yellow]%}.)$user@%m"
  fi
}
